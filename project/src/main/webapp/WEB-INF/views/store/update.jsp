...
<body>
				...
				<div class="col-md-12">
					<div class="block text-center">
						...
						<ul class="list-inline">
							<li class="list-inline-item"><span class="text-white-50">사업자용</span></li>
						</ul>
					</div>
				</div>
			</div>
	...
	<section class="section bg-white">
		<div class="container1">
			<form action="/store/update" method="POST" id="regForm" onsubmit="return checkAll();">

				<!-- 히든 인풋 -->
				<input type="hidden" name="dob" id="dob" value="${storeVo.dob }">
				<input type="hidden" name="email_flag" id="email_flag" value="${storeVo.email_flag }">
				<input type="hidden" name="sms_flag" id="sms_flag" value="${storeVo.sms_flag }">
				<input type="hidden" name="point" id="point" value="${storeVo.point }">
				<input type="hidden" name="rank" id="rank" value="${storeVo.rank }">
				<input type="hidden" name="picture" id="picture" value="${storeVo.picture }">
				<input type="hidden" name="open_hour" id="open_hour" value="${storeVo.open_hour }">
				<input type="hidden" name="close_day" id="close_day" value="${storeVo.close_day }">
				<input type="hidden" name="order_way" id="order_way" value="${storeVo.order_way }">
				<input type="hidden" name="delivery_fee" id="delivery_fee" value="${storeVo.delivery_fee }">
				<input type="hidden" name="delivery_area" id="delivery_area" value="${storeVo.delivery_area }">
				<input type="hidden" name="store_picture" id="store_picture" value="${storeVo.store_picture }">
				...
				<div class="register">
					...
					<div class="acc">
						...
						<input type="text" name="id" id="id" class="form_input" value="${storeVo.id }" readonly><br><span id="id_text"
						...
						<input type="text" name="nickname" id="nickName" class="form_input" onblur="check_nickName();" value="${storeVo.nickname }"><br><span
					...
					<div class="info">
						...
						<input type="text" id="year" maxlength="4" placeholder="년(4자)" class="form_input_30" onblur="check_dob();" value="${fn:substring(storeVo.dob,0,4) }">
						<select id="month" class="form_input_30" onchange="check_dob();">
							<option value="" selected>월</option>
							<option value="01" ${fn:substring(storeVo.dob,4,6) eq '01' ? 'selected' : '' }>1</option>
							<option value="02" ${fn:substring(storeVo.dob,4,6) eq '02' ? 'selected' : '' }>2</option>
							<option value="03" ${fn:substring(storeVo.dob,4,6) eq '03' ? 'selected' : '' }>3</option>
							<option value="04" ${fn:substring(storeVo.dob,4,6) eq '04' ? 'selected' : '' }>4</option>
							<option value="05" ${fn:substring(storeVo.dob,4,6) eq '05' ? 'selected' : '' }>5</option>
							<option value="06" ${fn:substring(storeVo.dob,4,6) eq '06' ? 'selected' : '' }>6</option>
							<option value="07" ${fn:substring(storeVo.dob,4,6) eq '07' ? 'selected' : '' }>7</option>
							<option value="08" ${fn:substring(storeVo.dob,4,6) eq '08' ? 'selected' : '' }>8</option>
							<option value="09" ${fn:substring(storeVo.dob,4,6) eq '09' ? 'selected' : '' }>9</option>
							<option value="10" ${fn:substring(storeVo.dob,4,6) eq '10' ? 'selected' : '' }>10</option>
							<option value="11" ${fn:substring(storeVo.dob,4,6) eq '11' ? 'selected' : '' }>11</option>
							<option value="12" ${fn:substring(storeVo.dob,4,6) eq '12' ? 'selected' : '' }>12</option>
						</select>
						<input type="text" id="day" maxlength="2" placeholder="일" class="form_input_30"
							onblur="check_dob();" value="${fn:substring(storeVo.dob,6,8) }"><br><span id="dob_text" class="check_text"></span>

						<h3><label for="" class="form_label">성별</label></h3>
						<select name="gender" id="" class="form_input">
							<option value="N" ${storeVo.gender eq 'N' ? 'selected' : '' }>선택안함</option>
							<option value="M" ${storeVo.gender eq 'M' ? 'selected' : '' }>남자</option>
							<option value="F" ${storeVo.gender eq 'F' ? 'selected' : '' }>여자</option>
						</select><br>
						...
						<input type="text" name="email" id="email" placeholder="선택입력" class="form_input"
							onblur="check_email();" value="${storeVo.email }"><br><span id="email_text" class="check_text"></span>
					</div>

					<!-- 휴대전화 -->
					<div>
						<h3><label for="hp" class="form_label">휴대전화</label></h3>
						<input type="tel" name="tel" id="tel" maxlength="11" placeholder="전화번호 입력" onblur="check_tel();" class="form_input" value="${storeVo.tel }" readonly>
					</div>
					...
					<div class="member">
						...
						<input type="text" name="addr" id="addr" class="form_input" readonly placeholder="주소" value="${storeVo.addr }">
						<input type="text" name="addr1" id="addr1" class="form_input" placeholder="상세주소" onblur="check_addr();" value="${storeVo.addr1 }"><br><span id="addr_text" class="check_text"></span>
					</div>
					<!-- END 일반회원 -->
					...
					<div class="company">
						...
						<input type="text" name="store_name" id="store_name" class="form_input" onblur="check_store_name();" value="${storeVo.store_name }"><br><span id="store_name_text" class="check_text"></span>
						...
						<input type="text" name="store_addr" id="store_addr" class="form_input" readonly placeholder="주소" value="${storeVo.store_addr }">
						<input type="text" name="store_addr1" id="store_addr1" class="form_input" placeholder="상세주소" onblur="check_addr();" value="${storeVo.store_addr1 }"><br><span id="store_addr_text" class="check_text"></span>
				...
				<div class="term">
					<div class="term_main">
						<label><input type="checkbox" name="" id="checkBoxAll" ${(storeVo.email_flag eq 'Y' && storeVo.sms_flag eq 'Y') ? 'checked' : '' }><span>&emsp;약관 모두 동의하기</span></label>
					</div>
					<div class="term_flag">
						<div class="flexbox">
							<label><input type="checkbox" name="" id="email_flag_check" class="checkBoxOne" ${storeVo.email_flag eq 'Y' ? 'checked' : '' }>&emsp;이벤트/쇼핑혜택 이메일 수신<span>&nbsp;(선택)</span></label>
					...
					<div class="term_flag">
						<div class="flexbox">
							<label><input type="checkbox" name="" id="sms_flag_check" class="checkBoxOne" ${storeVo.sms_flag eq 'Y' ? 'checked' : '' }>&emsp;이벤트/쇼핑혜택 SMS 수신<span>&nbsp;(선택)</span></label>
							<button type="button" class="term_toggle">내용보기</button>
						</div>
						<div class="term_text">
							<div class="term_area">
